...
function loadEmployees(){
  try {
    ...
    if(Array.isArray(x)&&x.length) return x.map(e=>({...e,edgeRound:0}));
  } catch(e){}
  return cloneDefaultEmployees().map(e=>({...e,edgeRound:0}));
}
function loadState(){
  ...
  if(imported?.length){
    ...
    for(const x of imported){
      ...
      if(prev) next.push({...prev,name:x.name,category:x.category});
        ...
        let mode=(x.category==="MAGAZYN"||x.category==="SERWIS")?"manual":"prodio", workRound=mode==="prodio"?15:0;
        next.push({name:x.name,category:x.category,active:true,mode,edgeRound:0,workRound});
      }
    }
    employees=next;
  }
